...
|---|---|---|---|
| 1 | Mutable image tags | `vaultwarden:latest`, `linkwarden:latest`, `traefik:v3`, `immich:release`, `dbgate:latest` — any can silently break or introduce CVEs on next pull. Pin to version tags (e.g., `v3.2`, `17-alpine`), not `latest`. DockHand handles update detection. | ✅ Fixed — pinned vaultwarden 1.36.0, linkwarden v2.14.1, traefik v3.7, immich v3.0.0 |
...
| 6 | Binaries downloaded without checksums | step-ca and kopia binaries downloaded from `latest/` URLs with no hash verification. Compromised release = backdoored binaries with no detection. | ✅ Fixed — pinned versions + added SHA256 checksum verification |
...
| 8 | Kopia password in logs | `kopia_server/tasks/deploy.yml` has `no_log: true` commented out on repo init. `KOPIA_PASSWORD` env var leaks to task output. Same on multiple kopia CLI commands. | ✅ Fixed — uncommented `no_log: true` on repo init |
...
| 16 | `evaluate=True` in `regex_search` | `tuning.yml:215` uses `evaluate=True` in `regex_search` — not a valid parameter per docs. May be silently ignored or cause errors. Fix: use `\\1` for capture groups. | ✅ Fixed — replaced with `\\1` capture group |
...
| 24 | adblock-lean from raw GitHub URL | Downloaded from `raw.githubusercontent.com` with no hash or version pin. Compromised repo = compromised router DNS. | ✅ Fixed — pinned to v0.8.1 |
...
| 26 | step-ca wildcard `*.lan` policy | Wildcard cert policy means a single compromised CA private key (stored on every server) validates every LAN service. Per-service certs limit blast radius. | ✅ Fixed — `step_ca_policy_wildcards` is already empty (`[]`), no wildcard policy active |
